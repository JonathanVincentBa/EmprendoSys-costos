...
<body class="min-h-screen bg-white dark:bg-zinc-800 antialiased">
    ...
    <div class="flex min-h-screen w-full">
        ...
        <flux:sidebar sticky collapsible="mobile"
            ...
            <flux:sidebar.nav>
                ...
                <flux:sidebar.group :heading="__('Costos de Producción')" class="grid">
                    <flux:sidebar.item icon="sparkles" :href="route('product.wizard')" wire:navigate>
                        Asistente Maestro
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="cube" :href="route('products.index')" wire:navigate>
                        Productos
                    </flux:sidebar.item>
                </flux:sidebar.group>
                ...
                <flux:sidebar.group :heading="__('Ventas')" class="grid">
                    <flux:sidebar.item icon="shopping-cart" :href="route('sales.pos')" wire:navigate>
                        Punto de Venta
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="user-group" :href="route('clients.index')" wire:navigate>
                        Clientes
                    </flux:sidebar.item>
                </flux:sidebar.group>

                {{-- Sección Configuración (Catálogos) --}}
                <flux:sidebar.group :heading="__('Configuración')" class="grid">
                    <flux:sidebar.item icon="beaker" :href="route('raw-materials.index')" wire:navigate>
                        Materias Primas
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="archive-box" :href="route('packaging.index')" wire:navigate>
                        Empaques
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="bolt" :href="route('supplies.index')" wire:navigate>
                        Suministros
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="calculator" :href="route('overhead-config.index')" wire:navigate>
                        Costos Indirectos
                    </flux:sidebar.item>
                    <flux:sidebar.item icon="identification" :href="route('labor-costs.index')" wire:navigate>
                        Mano de Obra
                    </flux:sidebar.item>
                </flux:sidebar.group>
            </flux:sidebar.nav>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <flux:header class="lg:hidden bg-zinc-50 dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-700 px-4 flex items-center h-16 sticky top-0 z-10">
